...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        suggestions = User.objects.exclude(pk__in=followings).exclude(is_superuser=True).exclude(username=request.user.username).order_by("?")[:6]
...
def logout_view(request):
    logout(request)
    return HttpResponseRedirect(reverse("intropage"))
...
def saved(request):
    if request.user.is_authenticated:
        ...
        followings = Follower.objects.filter(followers=request.user).values_list('user', flat=True)
        suggestions = User.objects.exclude(pk__in=followings).exclude(username=request.user.username).exclude(is_superuser=True).order_by("?")[:6]
        return render(request, "network/index.html", {
            "posts": posts,
            "suggestions": suggestions,
            "page": "saved"
...
def contact(request):
    ...
    return render(request, 'network/contactUs.html')
...
def text_analysis(request):
    if "submit-button" in request.POST:
        ...
        if len(user_input) < 3:
            return render(request, "network/text_analysis.html", {"error": "Please enter atleast 3 characters!"})
        
        result = a.calculating(user_input)
        sentence1 = "Your context displays "
        sentence2= " emotions !"
        return render(request, "network/text_analysis.html",{"result": result, "sentence1": sentence1, "sentence2": sentence2})
    
    return render(request, "network/text_analysis.html")
